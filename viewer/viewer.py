...
class Viewer:
    ...
    def init_scene(
        ...
        for idx in image_indices:
            ...
            # )
            filename = train_dataset.image_filenames[idx]
            file_format = filename.suffix[1:].lower()

            # torchvision can be slow to import, so we do it lazily.
            import torchvision

            if file_format.endswith('exr'):
                image_float32 = image.detach().type(torch.float32)
                image_float32 = image_float32.permute(2, 0, 1)

                image_float32 = torchvision.transforms.functional.resize(image_float32, 100, antialias=None)  # type: ignore
                image_float32 = image_float32.permute(1, 2, 0)
                image_float32 = image_float32.cpu().numpy()

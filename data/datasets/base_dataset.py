...
class InputDataset(Dataset):
    ...
    def get_numpy_image(self, image_idx: int) -> npt.NDArray[np.uint8]:
        ...
        if self.scale_factor != 1.0:
            ...
            newsize = (int(width * self.scale_factor), int(height * self.scale_factor))
            # pil_image = pil_image.resize(newsize, resample=Image.BILINEAR) # JO comment: try Image.BICUBIC -> for higher quality image
            # hdr_image = hdr_image.resize(newsize, resample=Image.BILINEAR) #Jo: this is for hdr
            image = image.resize(newsize, resample=Image.BILINEAR) # Jo: have a general format
        
        file_format = image_filename.suffix[1:].lower() 
